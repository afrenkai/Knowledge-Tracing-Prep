...
class LSTM():
    ...
    def sigmoid (self, x):
        return 1/ (1 + np.exp(-x))
    # sigmoid prime. fun little diversion https://math.stackexchange.com/questions/4620347/why-is-the-derivative-of-the-sigmoid-function-equivalent-to-x-1-x
    @staticmethod
    ...
    def forward(self, x):
        ...
        for t in range(len(x)):
            x_tr = x[t].reshape(-1, 1)  # Transpose input to column vector

            # forget gate forward pass (sigmoid(\cdot W_f (current weight matrix of forget gate), X^T (x transposed)) + \cdot U_f (weight matrix of previous state in forget gate), current hidden state ) + b_f (bias for forget gate)
    ...
    def backward(self, x, output_dim, eta):
        ...
        for t in reversed(range(len(x))):
            ...
            do_t = output_dim[t].reshape(-1, 1) * self.tanh(self.c_seq[t])
            ...
            dU_i += np.dot(di_t_raw, self.h_seq[t-1] if t > 0 else np.zeros_like(self.h).T)
            db_i += di_t_raw
...
class DeepKnowledgeTracing():
    ...
    def train(self, sequences, labels, epochs, eta: float = 0.01):
...
class TestDKT(unittest.TestCase):
    def setUp(self):
        ...
        self.sequences = [np.random.randint(0, 2, (self.sequence_length, self.num_skills * 2)) for _ in
                          range(self.batch_size)]
        self.labels = [np.random.randint(0, 2, (self.sequence_length, self.num_skills)) for _ in
                       range(self.batch_size)]

    def test_fwd_pass(self):
        predictions = self.dkt.predict(self.sequences[0])
        self.assertEqual(predictions.shape, (self.sequence_length, self.hidden_size))
